...
loadJSON((content) => {
    ...
    for (var [key, value] of map.entries()) {
        ...
        for (var label of labels) {
            if (!labelCounts.get(label)) {
                ...
                labelCounts.set(label, c);
            }
            var agreement = ((value[0].labels.indexOf(label) > -1 && value[1].labels.indexOf(label) > -1) ? 1 : 0);
            var counts = labelCounts.get(label);
    ...
    for (var [key, value] of labelCounts.entries()) {
        counts.push({label: key, counts: value});
    }
    ...
    for (var count of counts) {
        percentages.push({
            label: count.label,
            agreementPercentage: (count.counts.agreementDocuments + count.counts.agreementSummaries) / (count.counts.totalDocuments + count.counts.totalSummaries),
            agreementTotal: count.counts.agreementDocuments + count.counts.agreementSummaries,
    ...
    dlAnchorElem.setAttribute("download", "agreement_statistics.json");
    dlAnchorElem.click();
});